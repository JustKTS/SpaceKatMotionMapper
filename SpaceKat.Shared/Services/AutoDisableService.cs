...
public class AutoDisableService : IAutoDisableService
{
    private static readonly ILogger Log = Serilog.Log.ForContext<AutoDisableService>();
    ...
    public bool IsEnable
    {
        ...
        set
        {
            ...
            {
                Log.Warning("Cannot enable: platform not supported");
                return; // 阻止启用
            }

            if (value)
            {
                Log.Information("Auto-disable enabled");
                _localSettingsService.SaveSettingAsync(AutoDisableKey, value).ContinueWith(t =>
                {
                    if (t.IsFaulted) Log.Error(t.Exception, "Failed to save IsEnable setting");
                });
                _currentForeProgramHelper.ForeProgramChanged += ForeProgramChangedHandle;
            }
            else
            {
                Log.Information("Auto-disable disabled");
                _currentForeProgramHelper.ForeProgramChanged -= ForeProgramChangedHandle;
            }
    ...
    public async Task InitializeAsync()
    {
        ...
        {
            Log.Warning("Platform does NOT support foreground program monitoring");
            Log.Warning("Auto-disable feature will be disabled");
        }
        else
        {
        ...
        var keys = await _localSettingsService.ReadSettingAsync<List<string>>(AutoDisableProgramsKey).ConfigureAwait(false);
        if (keys is not null)
        {
            // 向后兼容：旧数据只有路径，没有进程名
            ...
            Log.Information("Loaded {Count} programs from settings", _autoDisablePrograms.Count);
        }

        // 只有在平台支持时才加载启用状态
        var isEnable = await _localSettingsService.ReadSettingAsync<bool?>(AutoDisableKey).ConfigureAwait(false);
        if (isEnable is not null)
        {
            // 强制检查：如果平台不支持，即使设置中保存了启用状态，也强制禁用
            if (!IsPlatformSupported)
            {
                Log.Warning("Platform not supported, forcing IsEnable to false");
    ...
    public void AddProgramPath(string programPath, string processName = "")
    {
        ...
        Log.Debug("Added program: Path={Path}, ProcessName={ProcessName}",
            programPath, processName);
        SavePrograms();
    }
    ...
    public void RemoveProgramPath(string programPath)
    {
        ...
        Log.Debug("Removed program: {Path}", programPath);
        SavePrograms();
    }
    ...
    {
        Log.Debug("ForeProgram changed: ProcessName={ProcessName}, Path={Path}, Title={Title}",
            info.ProcessName, info.ProcessFileAddress, info.Title);

        // 使用新的匹配逻辑
        var isContained = _autoDisablePrograms.Any(programInfo =>
            programInfo.Matches(info));

        Log.Information("Is in disable list: {IsContained}, Mapper enabled: {MapperEnabled}",
    ...
    private void SavePrograms()
    {
        ...
        {
            if (t.IsFaulted) Log.Error(t.Exception, "Failed to save auto-disable programs");
        });
    }
}