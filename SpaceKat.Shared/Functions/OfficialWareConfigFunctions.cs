...
public static class OfficialWareConfigFunctions
{
    ...
    private const string DisableButtonXml = 
        """
    ...
    public static async Task<Result<bool>> OpenOfficialMapper(bool clearButtons = false)
    {
        ...
        if (clearButtons)
        {
            var node3 = node1.SelectSingleNode("Devices")?.SelectSingleNode("Device")?.SelectSingleNode("ButtonBank");
            if (node3 == null) return true;
            node1.SelectSingleNode("Devices")?.SelectSingleNode("Device")?.RemoveChild(node3);
        }
