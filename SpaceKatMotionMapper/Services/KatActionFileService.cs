...
public class KatActionFileService
{
    ...
    public KatActionFileService(IFileService fileService)
    {
    ...
    public Result<bool> SaveDefaultConfigGroup(KatActionConfigGroup configGroup)
    {
        try
        {
    ...
    public Result<KatActionConfigGroup> LoadDefaultConfigGroup()
    {
        try
        {
            var ret = _fileService.Read<KatActionConfigGroup>(_configDirPath, DefaultConfigFileName);
            return ret ?? new Result<KatActionConfigGroup>(new Exception("读取配置文件失败"));
        }
        catch (Exception e)
        {
            return new Result<KatActionConfigGroup>(new Exception("读取配置文件失败", innerException: e));
        }
    }

    public Result<KatActionConfigGroup> LoadConfigGroup(string configFilePath)
    {
        try
        {
            var dir = Path.GetDirectoryName(configFilePath);
            if (dir is null) return new Result<KatActionConfigGroup>(new Exception("输入地址有误，无法提取文件夹"));
            var fileName = Path.GetFileName(configFilePath);
            var configGroup = _fileService.Read<KatActionConfigGroup>(dir, fileName);
            return configGroup ?? new Result<KatActionConfigGroup>(new Exception("读取配置文件失败"));
        }
        catch (Exception e)
        {
            return new Result<KatActionConfigGroup>(new Exception("读取配置文件失败", innerException:e));
    ...
    public Result<List<KatActionConfigGroup>> LoadConfigGroupsFromSysConf()
    {
        try
        {
            var configFiles = Directory.GetFiles(_customConfigDirPath, "*.json");
            
            List<KatActionConfigGroup> configGroups = [];
        ...
        {
            return new Result<List<KatActionConfigGroup>>(new Exception("读取配置文件错误", innerException:e));
        }
    }

    public Result<bool> SaveConfigGroupToSysConf(KatActionConfigGroup configGroup)
    {
    ...
    public Result<bool> SaveConfigGroup(KatActionConfigGroup configGroup, string configFilePath)
    {
        try
        {
            var dir = Path.GetDirectoryName(configFilePath);
    ...
    public Result<bool> SaveConfigGroupsToSysConf(IEnumerable<KatActionConfigGroup> configGroups)
    {
        var retFlag = true;
