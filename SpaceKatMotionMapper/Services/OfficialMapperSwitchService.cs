...
public class OfficialMapperSwitchService(ITopLevelHelper topLevelHelper)
{
    ...
    public Result<bool> OpenOfficialMapper()
    {
    ...
    private Result<bool> UnbindHotKeyToKatButton()
    {
        var doc = LoadDocument();
        var node1 = doc.SelectSingleNode("Global");
        if (node1 == null) return new Result<bool>(new Exception("配置文件解析失败，请检查配置文件是否有被不正确的修改！"));
    ...
    private Result<bool> BindHotKeyToKatButton(
    ...
    public Result<bool> CloseOfficialMapper()
    {
        var doc = LoadDocument();
        var node1 = doc.SelectSingleNode("Global");
        if (node1 == null) return new Result<bool>(new Exception("配置文件解析失败，请检查配置文件是否有被不正确的修改！"));
        ...
        KatButtonEnum katButtonEnum)
    {
        ...
        {
            UnregisterHotKey(nativeHandle, HotKeyEventId);
            var ret = RegisterHotKey(nativeHandle, HotKeyEventId, modifierKeys, (int)hotKey);
            if (!ret) return new Result<bool>(new Exception("注册热键失败"));
            if (katButtonEnum != KatButtonEnum.None)
            {
                BindHotKeyToKatButton(katButtonEnum, useCtrl, useAlt, useShift, hotKey);
            }
    ...
    public Result<bool> UnregisterHotKeyWrapper()
    {
        ...
        if (handle.Handle is { } nativeHandle)
        {
            var ret = UnregisterHotKey(nativeHandle, HotKeyEventId);
            return ret;
        }
